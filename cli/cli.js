...
async function main() {
    ...
    const sideTasksControl = new AbortController();

    handleMissingHomeCoords(page, sideTasksControl.signal)
        .catch(handleAsFatalError);

    findHomePlanetIfAsked(page, sideTasksControl.signal)
        .catch(handleAsFatalError);
    ...
    while (true) {
        try {
            ...
            if (e.code !== 'ECONNREFUSED' &&
                !String(e).includes('CONNECTION_REFUSED'))
                throw e;
        }
...
async function handleMissingHomeCoords(page, signal) {
    try {
        await waitForSelectorForever(
            page,
            'text/Import account home coordinates? (y/n)',
            { signal }
        );
    }
    catch (e) {
...
async function findHomePlanetIfAsked(page, signal) {
    try {
        await waitForSelectorForever(
            page,
            'text/Press ENTER to find a home planet.',
            { signal }
        );
    }
    catch (e) {
...
async function forwardGameTerminalOutputToStdout(page) {
    await page.exposeFunction('DF_CLI_PRINT', (str) => { stdout.write(str); });
    await page.exposeFunction('DF_CLI_RM_LINE', () => { clearStdoutLine(); });
    await page.exposeFunction('DF_CLI_NEW_LINE', () => { stdout.write('\n'); });
    await page.evaluate(() => {
...
function getParameters() {
    ...
    if (typeof playerIndex === 'string')
        privateKey = getPrivateKeyFromPlayerIndex(playerIndex);
    if (!privateKey)
        privateKey = args.find(a => a.startsWith('0x'));
    if (!privateKey)
        privateKey = getPrivateKeyFromPlayerIndex(0);

    const [host, port] =
        args.find(a => a.includes(':'))?.split(':') ||
        [DEFAULT_HTTP_CLIENT_HOST, DEFAULT_HTTP_CLIENT_PORT]
